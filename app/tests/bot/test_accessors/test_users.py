...
class TestUserAccessor:
    ...
    async def test_get_user(self, store: Store, user_1: UserModel, user_request: dict):
    ...
    async def test_add_user_to_session(self, db_sessionmaker,
                                       store: Store,
        ...
        async with db_sessionmaker() as session:
            game_session = await session.scalar(
                    select(SessionModel)
                    .where(
                        SessionModel.chat_id == update_with_callback.message.chat.id_,
    ...
    async def test_stop_game_session(self, db_sessionmaker: async_sessionmaker[AsyncSession],
                                     store: Store,
                                     update: Update,
        ...
        async with db_sessionmaker() as session:
            ...
            assert game_session.in_progress == False

    async def test_get_amount_of_users_in_session(self, db_sessionmaker,
                                                  store: Store,
                                                  game_session_1,
                                                  user_1):
         async with db_sessionmaker() as session:
            ...
            users_amount = await store.user.get_amount_of_users_in_session(game_session_1.chat_id)

            assert len(game_session.users) == users_amount
            assert users_amount == 1

    async def test_get_winners_game_session_in_progress(self,
                                                        store: Store,
        ...
        assert (f"Топ-2:\n"
                ...
                f" 2. @{user_2.username} Побед: 0") == text

    async def test_get_winners_game_session_not_in_progress(self,
                                                            store: Store,
    ...
    async def test_get_all_in_game_users(self, db_sessionmaker: async_sessionmaker[AsyncSession],
                                         store: Store,
                                         game_session_with_two_users: SessionModel,
                                         user_1: UserModel,
                                         user_2: UserModel,
        ...
        users = await store.user.get_all_in_game_users(game_session_with_two_users.chat_id)
        ...
        async with db_sessionmaker() as session:
            user_session = await session.scalar(select(UserSession)
                                        .where(UserSession.id_ == user_session_2.id_))
    ...
    async def test_set_round_number(self,
                                    store: Store,
                                    game_session_1: SessionModel
                                    ):
    ...
    async def test_set_points_two_users(self, db_sessionmaker: async_sessionmaker[AsyncSession],
                                        store: Store,
                                        user_session_1: UserSession,
                                        user_session_2: UserSession):
        await store.user.set_points(user_session_1.id_, user_session_2.id_, 1, 0)
    ...
    async def test_set_points_one_user(self, db_sessionmaker: async_sessionmaker[AsyncSession],
                                       store: Store,
                                       user_session_1: UserSession,
                                       user_session_2: UserSession):
    ...
    async def test_get_winner_in_pair_first_user_winner(self, db_sessionmaker: async_sessionmaker[AsyncSession],
                                                        store: Store,
                                                        user_session_1: UserSession,
                                                        user_session_2: UserSession,
                                                        user_1: UserModel,
        ...
        await store.user.set_points(user_session_1.id_, user_session_2.id_, 1, 0)
        name = await store.user.get_winner_in_pair(user_session_1, user_session_2)

        assert name == user_1.display_name

    async def test_get_winner_in_pair_second_user_winner(self, db_sessionmaker: async_sessionmaker[AsyncSession],
                                                        store: Store,
                                                        user_session_1: UserSession,
                                                        user_session_2: UserSession,
                                                        user_1: UserModel,
        ...
        await store.user.set_points(user_session_1.id_, user_session_2.id_, 0, 1)
        name = await store.user.get_winner_in_pair(user_session_1, user_session_2)

        assert name == user_2.display_name

    async def test_get_winner_in_pair_draw(self, db_sessionmaker: async_sessionmaker[AsyncSession],
                                                        store: Store,
                                                        user_session_1: UserSession,
                                                        user_session_2: UserSession,
                                                        user_1: UserModel,
        ...
        await store.user.set_points(user_session_1.id_, user_session_2.id_, 1, 1)
        name = await store.user.get_winner_in_pair(user_session_1, user_session_2)
    ...
    async def test_get_all_in_progress_game_sessions(self,
                                                     store: Store,
                                                     game_session_1: SessionModel,
                                                     game_session_2: SessionModel):
        game_sessions = (await store.user.get_all_in_progress_game_sessions()).all()
    ...
    async def test_set_message_id_to_session(self,
                                             store: Store,
                                             game_session_1: SessionModel):
        game_session = await store.user.set_message_id_to_session(game_session_1.chat_id, 42)

        assert game_session.message_id == 42

    async def test_get_seconds_game_session_exists(self,
                               store: Store,
                               game_session_1: SessionModel):
        seconds = await store.user.get_seconds(game_session_1.id_)
        assert seconds == 15

    async def test_get_seconds_game_session_not_exists(self,
                               store: Store):
