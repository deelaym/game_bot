...
class TestAdminLoginView:
    ...
    async def test_bad_request_when_missed_email(
        self, cli: TestClient, config: Config
    ) -> None:
        response = await cli.post(
            "/admin.login", json={"password": config.admin.password}
        )
    ...
    async def test_bad_request_with_wrong_email(
        self, cli: TestClient, config: Config
    ) -> None:
        response = await cli.post(
            "/admin.login",
    ...
    async def test_bad_request_with_wrong_password(
        self, cli: TestClient, config: Config
    ) -> None:
        response = await cli.post(
            "/admin.login",
            json={"email": config.admin.email, "password": "querty"},
        )
